...
async def GitCheckout(repo_path, git_ref, build_id):
    with Session() as session:
        ...
        # cleanup local changes
        if not await run_git("git reset --hard origin", repo_path, build, write_output_log=False):
            return False
        # delete old tag, update moved tags, fetch new tags
        if not await run_git("git fetch --prune origin +refs/tags/*:refs/tags/*", repo_path, build, write_output_log=False):
            return False
        # checkout

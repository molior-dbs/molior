...
export async function fetchRepos({ q = '', filter_url = '', page = 1, page_size = 25 } = {}) {
  ...
  const res = await fetch(`/api2/repositories?${params}`, { credentials: 'same-origin' });
  if (!res.ok) throw new Error(`${res.status}`);
  return res.json(); // { total_result_count, results }
}

export async function fetchRepo(id) {
  const res = await fetch(`/api2/repository/${id}`, { credentials: 'same-origin' });
  if (!res.ok) throw new Error(`${res.status}`);
  return res.json();
}
...
export async function fetchRepoDependents(id, { q = '', page = 1, page_size = 25 } = {}) {
  ...
  const res = await fetch(`/api2/repository/${id}/dependents?${params}`, { credentials: 'same-origin' });
  if (!res.ok) throw new Error(`${res.status}`);
  return res.json(); // { total_result_count, results }
}
...
export async function editRepoUrl(id, url) {
  const res = await fetch(`/api2/repository/${id}`, {
...
export async function deleteRepo(id) {
  const res = await fetch(`/api2/repository/${id}`, { method: 'DELETE', credentials: 'same-origin' });
  if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error(e.detail || res.status); }
}

export async function buildRepo(id) {
  const res = await fetch(`/api/repositories/${id}/build`, { method: 'POST', credentials: 'same-origin' });
  if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error(e.detail || res.status); }
}

export async function recloneRepo(id) {
  const res = await fetch(`/api/repositories/${id}/clone`, { method: 'POST', credentials: 'same-origin' });
  if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error(e.detail || res.status); }
}

export async function mergeRepo(originalId, duplicateId) {
  const res = await fetch(`/api2/repository/${originalId}/merge`, {
    method: 'PUT', credentials: 'same-origin',
    headers: { 'Content-Type': 'application/json' },

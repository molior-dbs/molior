...
CMD echo "export function MoliorWebVersion() { return \"`dpkg-parsechangelog -S Version`\"; }" > /app/src/app/lib/version.ts; PATH=node_modules/.bin:$PATH NODE_OPTIONS=--openssl-legacy-provider ng serve --poll=2000 --host 0.0.0.0 --base-href=/ --serve-path=/ --proxy-config docker/ng-serve.proxy.conf.json --disable-host-check || cat /tmp/ng-*/angular-errors.log

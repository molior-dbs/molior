...
RUN curl -s http://molior.info/1.5/archive-keyring.asc | gpg --dearmor -o /etc/apt/trusted.gpg.d/molior.gpg && apt-get update && \
    apt-get install -y --no-install-recommends apg bzip2 xz-utils ca-certificates golang/bookworm-backports golang-go/bookworm-backports golang-doc/bookworm-backports golang-src/bookworm-backports && \
...
CMD usermod -u `stat -c %u /app` aptly; \
    ...
    /app/debian/pkgdata/usr/sbin/create-aptly-keys $REPOSIGN_NAME $REPOSIGN_EMAIL && \
    su aptly -c "air -build.pre_cmd \"go mod tidy\" -build.exclude_dir system -- api serve -listen 0.0.0.0:3142"

...
RUN curl -s http://molior.info/1.5/archive-keyring.asc | gpg --dearmor -o /etc/apt/trusted.gpg.d/molior.gpg && apt-get update && apt-get install -y --no-install-recommends molior-aptly && apt-get clean && rm -rf /var/lib/apt/lists/*
